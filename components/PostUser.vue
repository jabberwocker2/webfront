<template>
    <div @click="displayPost(post)" @mouseenter="isHover(true)" @mouseleave="isHover(false)"
        ...
        <div v-if="!isLoaded"
            class="absolute flex items-center justify-center top-0 left-0 aspect-[3/4] w-full object-cover rounded-md border">
           <p class="text-gray-700 text-[25px] pt-1 break-words">{{ post.text }}</p> 
        </div>
